	...
	while (++column && (read = getc(file)) != EOF) {
		...
		switch (stage) {
			...
			case 4: {
				...
				stage = 0;
				moveTo = read - 48 - 1;
				// We decrement because we'd reset it anyway.
				for (; moveCount > 0; moveCount--) {
					Block* target = stacks[moveFrom];
					// We've reached the end of the list
					if (!target) {
						moveCount = 0;
						break;
					}
					// Pop from front and add to front of other stack
					stacks[moveFrom] = target->next;
					target->next = stacks[moveTo];
					stacks[moveTo] = target;
				}
				break;
			}
		}
